            ...
            while True:
                ...
                if 'article' not in ret:
                    ...
                    if article != None:
                        ...
                        url2 = urllib.parse.unquote(str(a['href']))
                ...
                if subcategories_div != None:
                    subcategories_text = subcategories_div.find('p')
                    assert subcategories_text is not None
                    subcategories_text = subcategories_text.string
                    assert subcategories_text is not None
                ...
                pages_cnt = 0
                if pages_div != None:
                    pages_cnt_text = pages_div.find('p')
                    assert pages_cnt_text is not None
                    pages_cnt_text = pages_cnt_text.string
                    assert pages_cnt_text is not None
                ...
                next = soup.find_all(
                    'a', string='下一页'  # pyright: ignore[reportArgumentType]
                )
...
def crawl(root: str, path: str, filter_function=None):
    partial_path = path.replace('.json', '.partial.json')
    ret = load_json_or_none(partial_path) or {}
    if 'finish2' not in ret:
        try:
            parse_index(
                root,
                ret,

...
attrs = json.load(open("moegirl/attr_index.json", encoding="utf-8"))
chars = json.load(open("moegirl/char_index.json", encoding="utf-8"))
P = np.load(open("moegirl/analysis/intersection.npy", "rb"))
...
def calc_chi2(x, y):
    ...
    chi2 = (p11 * p00 - p10 * p01) / (
        pxx * pyy * (char_count - pxx) * (char_count - pyy)
    ) ** 0.5
    return chi2
...
# hair_color_attr = json.load(open('moegirl/hair_color_attr.json', encoding='utf-8'))
...
contain = np.zeros(shape=[attr_count, attr_count], dtype=np.bool_)
